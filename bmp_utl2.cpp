...
#include "bmp_utl2.h"
	...
	*/
	PFRAMEBUFFER bmp_load(const char* file_path)
	{
		...
		if (pfb->bih.biBitCount >= 1 && pfb->bih.biBitCount <= 8) {
			...
			if (pfb->bih.biClrUsed > 0) {
				pfb->pal_count = pfb->bih.biClrUsed;
			}
	...
	*/
	void bmp_end(PFRAMEBUFFER pfb)
	{
		...
		//パレット領域（Pallette buffer）を解放
		free(pfb->ppal);
		pfb->ppal = NULL;	//解放したのでNULLを入れておく
		//ピクセル領域（Pixel buffer）を解放
		free(pfb->ppx);
		pfb->ppx = NULL;	//解放したのでNULLを入れておく
		//ファイルパス名領域（File path）を解放
		free(pfb->file_path);
		pfb->file_path = NULL;	//解放したのでNULLを入れておく
		//BMP構造体領域（BMP）を解放
		free(pfb);
		pfb = NULL;	//解放したのでNULLを入れておく
	}	//bmp_end
